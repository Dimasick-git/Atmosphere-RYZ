...
RU: Сначала изменения, специфичные для Ryazhenka, затем оригинальный changelog
Atmosphère.
...
+ Добавлен экран загрузки после заставки в чат-стиле (печать по буквам, зелёные «OK»), модуль `boot`. Показывается только при первом запуске (флаг `sdmc:/atmosphere/startup/ryazhenka_boot_shown`); удали файл — покажется снова.
